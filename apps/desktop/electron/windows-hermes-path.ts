...
export interface ResolveVenvHermesCommandDeps {
  ...
  buildDesktopBackendEnv: (opts: { hermesHome: string; pythonPathEntries: string[]; venvRoot: string }) => Record<string, string>
  hermesHome: string
...
export function resolveVenvHermesCommand(
  ...
  if (!isWindows || !command || isCommandScript(command)) {return null}

  const resolved = resolvePath(String(command))

  if (!/^hermes(?:\.exe)?$/i.test(basename(resolved))) {return null}

  const scriptsDir = dirname(resolved)

  if (basename(scriptsDir).toLowerCase() !== 'scripts') {return null}
